...
def generate_noise():
    ...
    s = np.random.randint(600, 700)  # using s as a cut off point
    ...
    for k in range(300):    # the bars increase in size
        ...
        d += 30 * factor    # changes the lower step length of the line
        f += 30 * factor    # changes the upper step length of the line
        s += np.random.randint(30, 60)  # increase in the cut off point
        l = np.random.randint(5, 10)
        y += np.random.randint(l, l + 10)   # gives a slight upward trend to the bars, not completely symmetric
        b = np.random.randint(y - 75, y + 75) # adds randomization


        # choose local extents
        h = d + f   # set range for max/min values of x and y
        w = max(1, 2 * p)
        ymin = max(0, b - int(h // 2))  # affects max/min size of the bars
        ...
        if s > 1000:    # bars stop increasing
           break
    ...
    for w in range(100):    # bars start to decrease in size
        ...
        y -= np.random.randint(l - 20, l)   # slight downward trend
        ...
        # add scaled gaussian
        spec[ymin:ymax, xmin:xmax] += 20.0 * gauss  # tune amplitude
        if s < np.random.randint(300,400):    # the lines stop decreasing
            break
