...
def generate_noise():
    ...
    factor = np.random.randint(15,30) # random factor to give different sizes of the storm
    ...
    y = np.random.randint(20000, 21000)  # storm starts randomly in the range on y axis
    for r in range(10):
        s = 0  # using s as a cut off point
        for k in range(300):  # the bars increase in size
            x = np.random.randint(t, t + 3)  # picks a starting point for the bar on the x axis
            t += np.random.randint(2, 6)  # each bar moves over a random amount on the x axis
            p = np.random.randint(1, 6)  # thickness of the bar

            d = np.random.randint(60, 150) * factor  # changes the lower step length of the line
            f = np.random.randint(60, 150) * factor  # changes the upper step length of the line
            s += np.random.randint(15, 40)  # increase in the cut off point
            b = np.random.randint(y - 100, y + 100)  # adds randomization
    ...
    plt.title(f'Io-A: Factor {factor}')
    plt.colorbar()
    plt.show()
