...
def generate_noise():
    ...
    for r in range(10):
        ...
        for k in range(300):    # the bars increase in size
            ...
            t += np.random.randint(5, 10)  # each bar moves over a random amount on the x axis
            p = np.random.randint(1, 8)    # thickness of the bar

            d = np.random.randint(30, 80) * factor    # changes the lower step length of the line
            f = np.random.randint(30,80) * factor    # changes the upper step length of the line
    ...
    plt.title(f'Io-C: Factor {factor}')
    plt.colorbar()
    plt.show()
